document.addEventListener("DOMContentLoaded", () => {
    ...
    // 3. HIGHCHARTS - VISUALISASI DATA STATISTIK
    ...
    if (typeof Highcharts !== 'undefined') {
        ...
        // Chart Penduduk (Pie) - SELURUH TEKS/LABEL DIHILANGKAN
        if (document.getElementById('chart-penduduk')) {
            Highcharts.chart('chart-penduduk', {
                ...
                series: [{ data: [ { name: 'Laki-laki', y: 572, color: '#3B82C4' }, { name: 'Perempuan', y: 614, color: '#E11484' } ] }],
                credits: { enabled: false }
            });
        }

        // Chart Klasifikasi Usia (Grouped Bar)
        if (document.getElementById('chart-usia')) {
            Highcharts.chart('chart-usia', {
                ...
                yAxis: { title: { text: 'Jumlah Jiwa' }, gridLineColor: c.grid, labels: { style: { color: c.text } } },
                legend: { itemStyle: { color: c.text } },
                series: [
                    { name: 'Laki-laki', data: [12,33,47,55,42,28,52,36,49,46,37,33,25,26,25,26], color: '#3B82C4' },
                    { name: 'Perempuan', data: [26,43,57,45,36,46,42,38,41,45,50,46,26,29,19,25], color: '#E11484' }
                ],
                credits: { enabled: false }
            });
        }

        // Chart Mata Pencaharian (Bar)
        if (document.getElementById('chart-pekerjaan')) {
            Highcharts.chart('chart-pekerjaan', {
                chart: { type: 'bar', backgroundColor: 'transparent' },
                title: { text: null },
                xAxis: { categories: ['Karyawan Swasta', 'Wiraswasta', 'Petani', 'Buruh Harian Lepas', 'Buruh Tani'], labels: { style: { color: c.text } } },
                yAxis: { title: { text: 'Jumlah Pekerja' }, gridLineColor: c.grid, labels: { style: { color: c.text } } },
                legend: { enabled: false },
                plotOptions: { bar: { dataLabels: { enabled: true, format: '{y} Orang' } } },
                series: [{ name: 'Pekerja', data: [
                    { y: 229, color: '#7A3CD6' },
                    { y: 120, color: '#3B82C4' },
                    { y: 108, color: '#279B48' },
                    { y: 58, color: '#F99D26' },
        ...
        // Chart Kelayakan Rumah (Donut)
        if (document.getElementById('chart-rumah')) {
            Highcharts.chart('chart-rumah', {
                chart: { type: 'pie', backgroundColor: 'transparent' },
                title: { text: null },
                plotOptions: { pie: { innerSize: '70%', dataLabels: { enabled: false }, showInLegend: true } },
                legend: { itemStyle: { color: c.text } },
                series: [{ data: [ { name: 'Layak Huni', y: 304, color: '#279B48' }, { name: 'Tidak Layak', y: 12, color: '#E01A22' } ] }],
                credits: { enabled: false }
            });
        }
    }
